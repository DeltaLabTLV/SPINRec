...
The SPINRec method (stochastic path integration for neural recommender explanations) is a model-agnostic, post-hoc approach designed to explain the reasoning behind recommendations generated by recommender systems. By integrating along a path from a sampled baseline to the input user vector, SPINRec generates an explanation map that illustrates each aspect of the user data's contribution to the recommendation output.
...
It was evaluated using three publicly available benchmarks datasets: MovieLens1M, a subset of the Yahoo!Music dataset, and a subset of the Pinterest dataset. 
...
* **code**: contains several code notebooks:
  ...
  - metrics_discrete - contains code related to the concise mertics evlaution (the refined evaluation framework) for all explainers.
...
+ For LXR eval framework: All baselines' results were calculated using the 'metrics_baselines' notebook, metrics for the SPINRec approach and its variant were calculated using the dedicated notebooks.
+ For refined eval framework: all resutls were calculated using the 'metrics_discrete' notebook.
